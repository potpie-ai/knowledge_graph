...
import os 
from ai_helper import get_llm_client   , llm_call,print_messages
...
class FlowInference:
    ...
    def get_flow(self, endpoint_id, project_id):
        ...
        nodes_pro = neo4j_graph.find_outbound_neighbors(
            endpoint_id=endpoint_id, project_id=project_id, with_bodies=False
        )
    ...
    async def generate_overall_explanation(self, endpoint: Dict, user_id: str) -> str:
        ...
        if code != '':
            ...
            if explanation_row:
                ...
                return await self.generate_explanation(code), code_hash
        cursor.close()
        conn.close()
